...
import {
  ...
  workspaceIdSchema,
} from "../validation/worksspace.validate";
import { HTTPSTATUS } from "../config/http.config";
import {
  changeMemberRoleService,
  createWorkspaceService,
  deleteWorkspaceService,
  getAllCurrentUserIsMemberService,
...
export const getAllWorkspaceController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const { workspaces } = await getAllCurrentUserIsMemberService(userId);
...
export const getWorkspaceAnalyticsController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    return res.status(HTTPSTATUS.OK).json({
      message: "Workspace members retrieved sucessfully",
      analytics,
    });
  }
);
